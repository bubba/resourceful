...
theory. A monad is often sardonically defined as
\begin{quote}
...
\begin{enumerate}
\item A set
\item With an associative binary operation $\mu : X \times X \rightarrow X$
\item And an identity element\footnote{An identity element $\eta : X$
    might be more familiar, but it isomorphic to a mapping from the
    unit type to $X$.} $g : \mathbf{1} \rightarrow X$
\item That satisfy some properties (the monoid laws)
\end{enumerate}

So if monoids allow chaining together elements, then monads in
programming allow sequencing together computation. In our system we
will be using the more pragmatic Haskell definition of a monad that
uses bind rather than join. But the idea of sequencing remains the
same.
