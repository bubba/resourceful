...
\newcommand{\unit}{\ensuremath{\raisebox{-0.35mm}{\square}}}
...
$\lift{e}$ lifts a regular expression into a monad (\texttt{return} in Haskell), and $e \bind e'$ is
...
\begin{grammar}
  ...
  <type $\tau$> ::= $\unit$ | $\tau \rightarrow \tau'$ | $\tau \times \tau'$ | $\textsf{IO}_\rho \tau$
  
  <type scheme $\sigma$> ::= $\forall \alpha . \sigma$ | $\tau$

  <expression $e$> ::= $\unit$ | $e \times e'$ | $\pi_1 \ e$ $\pi_2 \ e$
...
We also introduce the typing rule for $\unit$ expressions, to give us a
concrete type, as well as a rule for products which are constructed
through $e \times e'$.
\begin{mathpar}
  \inferrule*[Right=Unit]{ }{\Gamma \vdash \unit : \unit} \and
...
introduced into the language with either $\lift{e}$
$\textsf{readFile}$ or $\textsf{readNet}$.
\begin{mathpar}
  \inferrule*[Right=Lift]{\Gamma \vdash e : \tau}{\Gamma \vdash \lift{e} : \IO_\rho \tau} \and
    \inferrule*[Right=ReadFile]{ }{\Gamma \vdash \mathsf{readFile} : \IO_{\mathsf{File}} \unit} \and
    \inferrule*[Right=ReadNet]{ }{\Gamma \vdash \mathsf{readNetwork} : \IO_{\mathsf{Network}} \unit}
\end{mathpar}
...
type $\IO_{\textsf{File} \cup \textsf{Net}} \unit$. But if \textsc{Bind}
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule*[Right=Proj2]{\Gamma \vdash e : \tau \times \tau'}{\Gamma \vdash \pi_2 : \tau'} \and

    \inferrule*[Right=Lift]{\Gamma \vdash e : \tau}{\Gamma \vdash \llbracket e \rrbracket : \IO_\sigma \tau} \and
    \inferrule*[Right=Bind]{\Gamma \vdash e : \IO_\sigma \tau' \\ \Gamma \vdash e' : \tau' \rightarrow \IO_\sigma
      \tau}{\Gamma \vdash e \bind e' : \IO_\sigma \tau} \\
    \inferrule*[Right=Conc]{\Gamma \vdash e_1 : \IO_\sigma \tau_1 \\ \Gamma \vdash e_2 : \IO_{\sigma'}
      \tau_2 \\ \sigma \cap \sigma' = \emptyset}
    {\Gamma \vdash e_1 \curlyvee e_2 : \IO_{\sigma \cup \sigma'} \ \tau_1 \times \tau_2} \\
    
    \inferrule*[Right=ReadFile]{ }{\Gamma \vdash \mathsf{readFile} : \IO_{\mathsf{File}} \unit} \and
    \inferrule*[Right=ReadNet]{ }{\Gamma \vdash \mathsf{readNet} : \IO_{\mathsf{Net}} \unit} \\

    \inferrule*[Right=Subsumption]{\Gamma \vdash e : \IO_\sigma \tau \\ \sigma \subtyp \sigma'}
    {\Gamma \vdash e : \IO_{\sigma'} \tau} \and
    \inferrule*[Right=Unit]{ }{\Gamma \vdash \unit : \unit}
  \end{mathpar}
...
cannot evaluate the $\unit$ type any further, therefore all $\unit$s
are considered values. The same goes for product types, but only if
both inner components are values themselves. $\unit \times \unit$ is a
finished value, but $f a \times \unit$ might still have evaluation left to
...
\begin{figure}
  \begin{mathpar}
    \boxed{\textsf{value} \ e} \\
    \inferrule{ }{ \textsf{value} \ \unit } \and
...
\begin{mathpar}
  \inferrule{ }{\lift{v} \bind e_2 \leadsto e_2 \ v}
\end{mathpar}

Unlike the semantic rule for \beta-reduction, there is no premise
enforcing that $\lift{v}$ is a value, since all lifted terms are
values anyway.

Our resourceful \textsf{readFile} and \textsf{readNet} primitives
return dummy unit values, lifted into the monadic form. The
concurrent operator $\curlyvee$ has the aforementioned $\xi$-reduction rules
that allow both of its arguments to reduce down until they reach
$\lift{v} \curlyvee \lift{w}$, at which point they can then be packaged up and
returned in a lifted product, $\lift{v \times w}$. In terms of dynamic
semantics, this is isomorphic to

$$\lift{v} \bind \lambda v . (\lift{w} \bind (\lambda w . \lift{v \times w}))$$

However, their static semantics are different. Namely, given $\Gamma \vdash
\lift{v} : \IO_\rho \tau$,
$$
...
premise $\rho \cup \rho = \emptyset$ does not hold.
$$\Gamma \nvdash v \curlyvee v$$
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule{\textsf{value} \ e_2}{ (\lambda x . e_1) e_2 \leadsto e_1 [ e_2 / x ] } \and
    ...
    \inferrule{ }{\textsf{readFile} \leadsto \lift{\unit}} \and
    \inferrule{ }{\textsf{readNet} \leadsto \lift{\unit}}
  \end{mathpar}
...
\begin{figure}
  \begin{center}
    ...
    \tikz \graph[layered layout, grow=right, edges={
      decoration={snake,amplitude=0.5mm,segment length=2mm,post length=1mm},decorate}] {     a -> b -> c
  };

  % give this two head arrows
  \tikz \graph[layered layout,grow=right] {
    a ->[orient] b -> c;
    a ->[bend right] c;
    { [same layer] a, b, c };
    a ->[loop above] a;
...
% Should have the type $\IO_{\mathsf{File} \cup \mathsf{Network}} \unit$
...
% \begin{align*}
%   \mathbb{V} &= \mathbb{B}_{\unit} + \mathbb{B}_{\textsf{Bool}} + \mathbb{F} +
%                \mathbb{W} \\
%   \mathbb{B}_{\unit} &= \{ \ \unit \ \} \\
...
\begin{proof}
  ...
  \item[\textmd{\boxed{\textsc{Unit}}}]
    We have $\Gamma \vdash \unit : \unit$. If $\unit = \tau'$ and $\tau' > \tau$, all
    substitutions on $\unit$ will result in $\unit$. Thus $\tau = \unit$, and we
    have already shown $\Gamma \vdash \unit : \unit$.
...
\begin{proof}
  ...
  \item[\textmd{\boxed{\textsf{readFile} \leadsto \lift{\unit}}}]
    We know $\Gamma \vdash
    \textsf{readFile} : \IO_{\textsf{File}} \unit$, and we can just use
    \textsc{Lift} to show that $\Gamma \vdash \lift{\unit} : \IO_{\textsf{File}}
    \unit$.
  \item[\textmd{\boxed{\textsf{readNet} \leadsto \lift{\unit}}}]
